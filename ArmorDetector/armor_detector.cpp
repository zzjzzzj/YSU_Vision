...
void ArmorDetector::ScreenArmor(){
    ...
    cout << match_armors_.size() << endl;
    cv::Point image_cen_p = cv::Point(src_image_.cols / 2, src_image_.rows / 2);
            ...
            if(match_armors_.size() != 1) // 如果当前帧检测到目标不唯一，则根据多目标优先算法进行排序。
            {
               if(hero_priority==0){ // 多因素混合打分
                    sort(match_armors_.begin(),match_armors_.end(),[=](const DetectRect & rect1, const DetectRect & rect2) {
                        ...
                        float ratio1 = (rect1.r_rect.size.width / rect1.r_rect.size.height);
                        float ratio2 = (rect2.r_rect.size.width / rect2.r_rect.size.height);
                        if(get_distance( rect1.cen_p, image_cen_p) < get_distance(rect2.cen_p, image_cen_p)) score1 += 1;
                        if(get_distance( rect1.cen_p, image_cen_p) > get_distance(rect2.cen_p, image_cen_p)) score2 += 1;
                   ...
                   cout << match_armors_.size() << endl;
                    ...
                    sort(match_armors_.begin(),match_armors_.end(),[](const DetectRect & rect1, const DetectRect & rect2) {
                        return rect1.r_rect.size.area() > rect2.r_rect.size.area();
                    });
                }
            }
