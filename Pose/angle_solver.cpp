...
inline void showForecast(cv::Point2f& src,std::vector<cv::Point2f>&src_obj,
                         ...
                         cv::Point2f& real,
                         cv::Mat &img){
    circle(img,real,5,Scalar(255,255,255));
...
double * AngleSolver::SolveAngle(std::vector<cv::Point2f>& observation_obj){
    ...
    loss_num = 0;
     ...
     auto &real = p_forecast_ ->getReal(middle.x,middle.y);
     auto &forecast_middel = p_forecast_ -> getNextForecast();
     int x_min_error=10;
     int x_max_error=100;
     int y_error=1;
     Point2f original_center=middle;
     cv::Point2f dst_real_point = cv::Point2f(real.at<float>(0), real.at<float>(1));
     cv::Point2f dst_forecast_point = cv::Point2f(forecast_middel.at<float>(0), forecast_middel.at<float>(1));
     if(abs(dst_forecast_point.x-dst_real_point.x)<x_min_error)
     {
         dst_forecast_point.x=dst_real_point.x;
     }

     if(dst_forecast_point.x-dst_real_point.x>x_max_error)
     {
         dst_forecast_point.x=dst_real_point.x+x_max_error;
     }
     ...
     if(dst_forecast_point.y-dst_real_point.y>y_error)
     {
         dst_forecast_point.y=dst_real_point.y+y_error;
     }
     if(dst_forecast_point.y-dst_real_point.y<-y_error)
     {
         dst_forecast_point.y=dst_real_point.y-y_error;
     ...
     getTranslationObj(middle, dst_real_point, observation_obj, object_armor_points_ );

//debug forecast
    cv::Mat img(1280,1024,CV_8UC3,cv::Scalar(0,0,0));
    showForecast(middle, observation_obj, dst_forecast_point, object_armor_points_, dst_real_point, img);
    ...
    double comp_angle=gravity_compensation(aim_pos, 34.1);//小弹丸射速15m/s   英雄为20   //*****************************调初速度


    p_y_err[0] = p_y_err[0];//-7.8 ;//yaw增加方向为左方向，伏视角逆时针方向   原0.6
    p_y_err[1]+=(-comp_angle+aim_angle)*180/CV_PI;//pitch
    ...
    std::cout << "pitch_res=" << p_y_err[1] << std::endl;
    std::cout << "yaw_res=" << p_y_err[0] << std::endl;
...
//下面是旧版的重力补偿
    ...
    if(distance_3d>GRACOMP_DIS)
    {
        p_y_err[0] = _xErr;
//       p_y_err[1] = _yErr;
        p_y_err[1] = _yErr-(CAM_SUPPORT_DIS+4.9*gra_t*gra_t)*0.0008;
