...
//            }
            ...
            temp_rect.confidence = confidence;
    ...
    if(rects.size()){
        ...
        for(unsigned int i = 0;i < rects.size() - 1;i++){
            for(unsigned int j = i + 1;j < rects.size();j ++){
                cv::Rect rect = rects[j].rect; // max p rect
                cv::Rect rect1 = rects[i].rect; // item p rect
                //计算两个矩形的交集
                Rect rect2 = rect | rect1;
                //计算两个举行的并集
                Rect rect3 = rect & rect1;
                //计算IOU
                double IOU_with_the_max = rect3.area() *1.0/ rect2.area();
                // 如果IOU过大就将其舍去
