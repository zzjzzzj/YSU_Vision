...
void threadManager::Communicate(){ //传递信息就直接修改p_communication_->Infantry中对应的数值即可
    int communit_base_time = static_cast<int>(1000000000/Communit_FPS_);
    int sleep_time;
    ...
    while(1)
    {
        ...
        auto end_time = std::chrono::system_clock::now();
        sleep_time = communit_base_time - (std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - start_time).count());
        if (sleep_time > 0) {
